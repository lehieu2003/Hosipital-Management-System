...
This project uses Codex AI as an intelligent development agent with structured workflows, specialized sub-agents, and mandatory coding standards.
...
All rules in `.Codex/rules/` are **mandatory** and must be followed:
...
Quick references in `.Codex/references/`:
...
2. **Apply mandatory rules** — All rules in `.Codex/rules/` are non-negotiable
